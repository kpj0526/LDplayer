...
## PRESERVE-LOCKED-SLOTS-001 — avoid refreshing surviving target missions after a claim (2026-09-23)

1. **Owner**: existing Code worktree implements/commits; existing QA independently verifies; Manager coordinates.
2. **Evidence**: rc.36 claims one completed mission, then `runtime.reset_after_verified_return()` sets all five slot states to `UNKNOWN`. The next configuration pass rechecks previously locked `모든 몬스터 처치` missions with an initial `0/200` condition; their in-progress N/200 objectives fail that condition and can be refreshed despite still being the target. The customer reports this after all reward completions.
3. **Change**: after verified reward close and mission-list return, mark only the claimed slot `UNKNOWN` and revisit it first. Preserve other `TARGET_LOCKED` slots and their per-account state. Retain explicit serial scope, stop checks, completion target verification, and guarded refresh for the actual replacement slot.
4. **Acceptance**: state transition preserves other locks; next cycle selects the replacement slot first and skips other locked slots; no refresh tap is sent to a surviving target with N/200 progress; full suite/build pass. Real customer game test and independent QA remain separately tracked.

## KEEP-NEW-TARGET-PHRASE-001 — do not refresh a newly assigned target mission (2026-09-23)

1. **User correction**: After completing and claiming a mission, the replacement mission visibly says `모든 몬스터 처치`, yet the worker refreshes it. This is the replacement-slot decision, not the state of the other four slots.
2. **Cause to address**: `assess_mission_target(require_initial_zero=True)` returns `NON_TARGET_CONFIRMED` immediately when the exact `0/200` template misses, before checking the calibrated, digit-free target phrase. This routes the new target to refresh.
3. **Implementation**: Code worktree must allow the configured real `mission_target_phrase` template to confirm a new target even when the exact initial-count template misses. Preserve the exact template as a positive signal, but do not make a particular count a negative signal for a positively verified objective. Do not accept unrelated missions or uncertain/capture-failed frames. Do not blindly tap an accept control on a plain completed/in-progress detail screen. Keep explicit serial and stop scope. Update tests using real customer captures for target/non-target and a post-claim replacement decision. Commit implementation and hand off to QA.
4. **Verification/status**: Run available automated tests and packaged self-test; independent QA and real LDPlayer/game confirmation remain pending and must not be declared PASS from simulated evidence.
