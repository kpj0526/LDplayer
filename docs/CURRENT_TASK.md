...
- Customer explicitly requested a test release before real-PC verification. Prerelease `v1.0.3-rc.35` was published from Code commit `ce64e96` with ZIP SHA-256 `9BFEBC2699BA2713D3DC8BAB122D7827E5D55E279D778CACA4467A9E1A04DB49` at https://github.com/kpj0526/LDplayer/releases/tag/v1.0.3-rc.35. Automated suite: 480 passed; isolated Windows build and packaged self-test passed. Independent QA and real LDPlayer result remain pending; this is not final PASS.
## COMPLETED-BEFORE-REFRESH-001 (2026-09-23)

- Customer rc.35 capture shows a selected `모든 몬스터 처치` mission with a `완료` button, not a refresh dialog. Real recognizer on the normalized customer image: target phrase 0.856, complete button 0.900, exact initial `0/200` 0.673. The initial-zero acceptance branch treated the completed target as non-target and sent the refresh-open tap on a plain completed screen.
- Code implementation `4ebd582` recognizes completed target before refresh and routes it through the existing guarded claim flow. Completed non-target/uncertain screens receive no refresh or reward input. Real frame regression and full 482-test suite pass; Windows build and packaged self-test pass.
- Customer requested an immediate test release. Prerelease `v1.0.3-rc.36` from Code commit `2047cb8` is published at https://github.com/kpj0526/LDplayer/releases/tag/v1.0.3-rc.36; ZIP SHA-256 `3B240250EED9F53675480B8ABE4E4C8297951478055371D88985609E0DFBF5FC`. Actual LDPlayer behavior and independent QA remain `NEEDS_REAL_TEST` / pending; no final PASS claim.
## PRESERVE-LOCKED-SLOTS-001 (2026-09-23)

- Customer reports the program and commit memory are stable on rc.36, but surviving `모든 몬스터 처치` missions are renewed after another mission's reward is claimed.
- Code cause: `reset_after_verified_return()` cleared all five slot locks. The next pass treated surviving N/200 targets as fresh missions and refreshed them when they failed the initial `0/200` check.
- Code implementation `0a75d17` clears only the claimed row and revisits that replacement first. Regression proves other locked rows with N/200 progress are not selected or refreshed. Full suite: 482 passed; isolated Windows build and packaged self-test passed.
- Customer test prerelease `v1.0.3-rc.37` from Code commit `013e503` is published at https://github.com/kpj0526/LDplayer/releases/tag/v1.0.3-rc.37. ZIP SHA-256 `B852896EFF23AE2B1CDCC3B9186518F98C6D44B2CDC7415B8E203A388CA7E32F`. Real customer result and independent QA remain pending; no final PASS claim.
